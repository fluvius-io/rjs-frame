    ...
    return (
      <div className="p-6 space-y-4">
        <div className="p-4 bg-gray-50 border rounded-md">
          <h3 className="text-lg font-semibold mb-2">Query Builder Modal Demo</h3>
          <p className="text-sm text-gray-600 mb-4">
            Click the button below to open the query builder modal and build some filters.
          </p>
      ...
      <div className="min-h-screen">
        ...
        <div className="p-6 space-y-6">
          <div className="mb-4 p-4 bg-gray-50 border rounded-md">
            <h3 className="text-lg font-semibold mb-2">Always Open Query Builder Modal</h3>
            <p className="text-sm text-gray-600">
              This story shows the modal always open for easier development and testing.
              Notice that the page background cannot be scrolled when the modal is open.
            </p>
          </div>
          ...
          {Array.from({ length: 50 }, (_, i) => (
            <div key={i} className="p-4 bg-white border rounded-md shadow-sm">
              ...
              <p className="text-sm text-gray-600 mt-2">
                This is sample content to make the page scrollable. When the modal is open,
                you should not be able to scroll this background content. Only the modal
                content itself should be scrollable if it overflows.
              </p>
            </div>
          ))}
        </div>
...
export const WithPrefilledFilters: Story = {
  render: () => {
    ...
    const [currentFilters, setCurrentFilters] = useState<QueryBuilderState>({
      ...
      sortRules: [
        { field: 'name__family', direction: 'asc' }
      ],
      filterRules: [
        {
          id: 'filter-1',
    ...
    return (
      <div className="p-6 space-y-4">
        <div className="p-4 bg-gray-50 border rounded-md">
          <h3 className="text-lg font-semibold mb-2">Pre-filled Query Builder Modal</h3>
          <p className="text-sm text-gray-600 mb-4">
            This story demonstrates the modal with some pre-existing filters and sort rules.
          </p>
      ...
      <div className="min-h-screen">
        <div className="p-6 space-y-6">
          ...
          {Array.from({ length: 100 }, (_, i) => (
            <div key={i} className="p-4 bg-white border rounded-md shadow-sm">
              <h4 className="font-medium text-gray-900">Scrollable Content Block {i + 1}</h4>
              <p className="text-sm text-gray-600 mt-2">
                This content should not be scrollable when the modal is open.
                The modal prevents background scrolling and only allows scrolling
                within the modal content area itself.
              </p>
              <div className="mt-2 text-xs text-gray-500">
                Block #{i + 1} • Modal Status: {isOpen ? 'Open (scroll locked)' : 'Closed (scroll enabled)'}
              </div>
            </div>
          ))}
        </div>
