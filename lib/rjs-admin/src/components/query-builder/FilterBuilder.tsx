...
const FilterBuilder = memo<FilterBuilderProps>(
  ({ metadata, filterRules, onFilterRulesChange }) => {
    ...
    const addCompositeGroup = useCallback(
      (operator: ".and" | ".or") => {
        ...
        onFilterRulesChange([...filterRulesRef.current, newCompositeFilter]);
      },
      [onFilterRulesChange]
    );
      ...
      <div className="space-y-2">
        ...
        <div className="border rounded-md p-3 space-y-4">
          ...
          {!hasFilters && availableFields.length > 0 && (
            <div className="p-2 text-sm text-gray-500 italic">
              No filters defined. All records will be returned (subject to other
              query parameters).
            </div>
          )}
