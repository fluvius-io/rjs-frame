...
const meta: Meta<typeof DataTable> = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component: 'Testing the QueryBuilder filter integration with DataTable.',
      },
    },
  },
  tags: ['autodocs'],
...
export const QueryBuilderFilters: Story = {
  render: () => {
    const [queryData, setQueryData] = useState<ResourceQuery & { searchQuery?: string } | null>(null);
    const [currentPage, setCurrentPage] = useState(1);
    const [pageSize, setPageSize] = useState(10);

    const handleQueryChange = useCallback((query: ResourceQuery & { searchQuery?: string }) => {
      setQueryData(query);
    }, []);
