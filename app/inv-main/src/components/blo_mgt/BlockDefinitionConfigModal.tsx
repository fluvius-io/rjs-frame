...
interface BlockDefinitionConfigModalProps {
  open: boolean;
...
export function BlockDefinitionConfigModal({ open, onClose, onRun }: BlockDefinitionConfigModalProps) {
  ...
  const [blockMeta, setBlockMeta] = useState<any>(null);
  ...
  useEffect(() => {
    ...
    setRunError("");
    try {
      APIManager.queryMeta("trade-manager:create-block-definition").then((response: { data: any }) => {
        setBlockMeta(response.data.schema);
        console.log("Block meta fetched successfully:", response.data.schema);
      });
    } catch (error) {
  ...
  const validateForm = (): boolean => {
    if (!blockMeta?.properties) return true;

    const errors: Record<string, string> = {};
    const requiredFields = blockMeta.required || [];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let response: any;
      try {
        response = await APIManager.send("trade-manager:create-block-definition", data, {path: {resource: 'block-definition', _id: ':new'}});
      } catch (error: any) {
        console.error("Failed to create block definition:", error);
        setRunError(error.message || "Failed to create block definition. Please try again.");
      }
      console.log("Block definition created successfully:", response);
      onRun(response.data.params);
      onClose({
        success: true,
        data: response.data,
        params: formData
  ...
  const renderInputField = (key: string, prop: any) => {
    const isRequired = blockDef?.params?.required?.includes(key);
            ...
            <>
              ...
              <div className="p-4 pt-0">
                <p className="mb-2 text-sm text-gray-700">
                  Fill in the required parameters to run the block.
                </p>
                ...
                <form className="w-full" onSubmit={handleSubmit}>
                  <table className="w-full">
                    <tbody>
                      {blockMeta?.properties && Object.entries(blockMeta?.properties).map(([key, prop], index) => {
                        const isRequired = blockMeta?.required?.includes(key);
                        return (
                          <tr key={key}>
                  ...
                  <div className="flex justify-end gap-2 pt-2">
                    ...
                    <button
                      ...
                      disabled={isRunning}
                    >
                      {isRunning ? "Creating Block Definition..." : "Run"}
                    </button>
                  </div>
                </form>
              </div>
