...
export const BlockListView: React.FC<BlockListViewProps> = ({ resourceName, profitSummary }) => {
    ...
    <div className="p-3 border bg-white/80 shadow-sm">
      <div className="flex items-center gap-2 px-2 pb-1 pt-1">
        <span className="font-semibold text-base text-gray-700 flex items-center gap-1">
          <span className="inline-block align-middle">🔲</span> Blocks ({pagination.total})
        </span>
            ...
            {blocks.map((block, idx) => (
              <React.Fragment key={block.id + idx}>
                <tr className="border-b border-gray-200 text-gray-400 font-semibold text-xs">
                  ...
                  <td className={`px-1 py-1 ${statusColor(block.status)} text-center`}>
                    {block.status}
                  </td>
                </tr>
              </React.Fragment>
